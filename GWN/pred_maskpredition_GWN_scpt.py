...
def testModel_estimation_with_pretrain(name, mode, test_iter, node_indices, scaler, adj_tst_u, mapping,spatialsplit):
    ...
    if P.IS_PRETRN:
        ...
        #with torch.no_grad():
            ...
            #node_embed = full_embed[:, node_indices] 
        n_node_remaining = P.N_NODE
        data_= data_ds if P.IS_DESEASONED else data
        with torch.no_grad():
            i=0
            while n_node_remaining > 2000:
                node_embed[:, i*2000:(i+1)*2000] = encoder(torch.Tensor(data_[:P.trainval_size,spatialsplit.i_tst[i*2000:(i+1)*2000]]).to(device).float().T).T.detach()
                i+=1
                n_node_remaining -= 2000
            node_embed[:, -n_node_remaining:] = encoder(torch.Tensor(data_[:P.trainval_size,spatialsplit.i_tst[-n_node_remaining:]]).to(device).float().T).T.detach()                                   # [H, N_sub]
    else:
