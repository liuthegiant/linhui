...
def getXSYS(data, mode):
    ...
    if mode == 'TRAIN':
        ...
        # ⭐ start at TRAIN_NUM to avoid overlap with the training time range.
        # Test windows now strictly live in [TRAIN_NUM, data.shape[0]).
        for i in range(TRAIN_NUM, data.shape[0] - P.TIMESTEP_OUT - P.TIMESTEP_IN + 1):
...
class ImputationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Supervise only artificially hidden observed values.
        loss_mask = extra  # [T, N, 1]

        x_masked = x.clone()

        # Input should hide both originally missing values and artificially masked observed values.
        input_mask = (m + extra).clamp(0.0, 1.0)
        mt = input_mask.squeeze(-1).permute(1, 0)  # [N, T]
...
def setups():
    ...
    # 因为 unseen_nodes.SpatialSplit 在 r_trn+r_val+r_tst 接近 1.0 时受浮点
    # 误差影响,可能误走 else 分支(独立有放回抽样),三个集合会互相重叠,
    # 也就是说 "未见" 测试节点里其实有相当一部分在训练里见过。
    ...
    _pct_vt  = len(_inter_vt) / max(len(_u_tst), 1) * 100.0
    print(f"[spatial:unseen] |trn|={len(_u_trn)} |val|={len(_u_val)} "
          f"|tst|={len(_u_tst)} sum={len(_u_trn)+len(_u_val)+len(_u_tst)} "
          f"N_total={data.shape[1]}")
    print(f"[spatial:unseen] trn ∩ tst : {len(_inter_tt)} 个 "
          f"({_pct_tst:.2f}% of tst; "
          f"也就是 {len(_inter_tt)}/{len(_u_tst)} 个 \"未见\"测试节点其实在训练里)")
    ...
    print(f"[spatial:unseen] val ∩ tst : {len(_inter_vt)} 个 ({_pct_vt:.2f}% of tst)")
    if _inter_vt:
        print(f"[spatial:unseen] val ∩ tst IDs: {_inter_vt}")

    # all-nodes split: 设计上 i_tst 就是全部节点(r_tst=1.0),所以训练节点
    # "全部"会出现在 test_a。这里也打印一下以备核对。
    _a_trn = set(int(x) for x in spatialSplit_allNod.i_trn)
    _a_tst = set(int(x) for x in spatialSplit_allNod.i_tst)
    _inter_at = sorted(_a_trn & _a_tst)
    _pct_at = len(_inter_at) / max(len(_a_tst), 1) * 100.0
    print(f"[spatial:allNod] |trn|={len(_a_trn)} |tst|={len(_a_tst)} "
          f"trn ∩ tst : {len(_inter_at)} 个 ({_pct_at:.2f}% of tst; "
          f"test_a 是 transductive setting,期望此比例较高)")
    # ---------------------------------------------------------------------
...
def graph_constructor_helper():
    ...
    # Q1 -> fQ1: feature matrix
    # Q1 -> nQ1: edge index, GCN doesn't like adjacency matrices testModel
...
def testModel(name, mode, test_iter, adj_tst, spatialsplit):
    ...
    #LOSS_MASK = extra.numpy()
    LOSS_MASK = test_iter.dataset.fixed_extra.cpu().numpy()
    ...
    # ============================================================
    # Metric computation (fixed):
    #   1) Exclude positions where the raw data was NaN — those positions had
    #      no real ground truth (they were nan_to_num'd to 0) and would
    #      otherwise dominate MAPE.
    #   2) For MAPE, only count samples whose true value is meaningfully
    #      non-zero (|gt| > eps). For speed-type traffic data eps=1.0 is fine.
    # ============================================================
    MS_orig = test_iter.dataset.M.cpu().numpy()                  # [B, T, N, 1]
    real_obs = ~np.squeeze(MS_orig).astype(bool)                 # True = originally observed
 
    mask2 = np.squeeze(LOSS_MASK).astype(bool) & real_obs        # [B, T, N]
    gt = YS_inv[mask2]
    pr = YP_inv[mask2]
    ...
    if gt.size == 0:
        ...
        # MAPE: only on samples whose true value is meaningfully non-zero.
        # Adjust eps to your data scale.
        eps = 1.0
        nz = np.abs(gt) > eps
        if nz.any():
            MAPE = float(np.mean(np.abs(err[nz]) / np.abs(gt[nz])))
            n_used_for_mape = int(nz.sum())
        else:
            MAPE = float('nan')
            n_used_for_mape = 0
 
        print(f'[metric debug] eval_pts={gt.size}, '
              f'gt range=[{gt.min():.3f}, {gt.max():.3f}], '
              f'gt|<={eps}: {(np.abs(gt) <= eps).sum()}/{gt.size}, '
              f'used for MAPE: {n_used_for_mape}')
...
class ResidualDeltaFusion(nn.Module):
    ...
    def forward(self, z1, z2):
        ...
        gate_logits = self.gate_net(feat).T  # [1, N] testModel
