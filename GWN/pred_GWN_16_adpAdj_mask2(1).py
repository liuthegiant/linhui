...
'''nohup python pred_GWN_16_adpAdj_mask2.py > pred_GWN_16_adpAdj_mask2.log 2>&1 &
disown'''
class StandardScaler: 
    ...
    def fit_transform(self, x):
        # NaN-safe: keep NaNs, stats computed on observed entries.
...
def getXSYS(data, mode):
    ...
    XS, YS, MS = [], [], []
    if mode == 'TRAIN':
        starts = list(range(TRAIN_NUM - P.TIMESTEP_OUT - P.TIMESTEP_IN + 1))
        for i in starts:
            # Imputation: target is the same window (no future forecasting).
            x = data[i:i+P.TIMESTEP_IN, :]  # [T, N]
            y = data[i:i+P.TIMESTEP_IN, :]  # [T, N]
            m = np.isnan(y).astype(np.float32)  # [T, N]
            XS.append(np.nan_to_num(x, nan=0.0))
            YS.append(np.nan_to_num(y, nan=0.0))
            MS.append(m)
    elif mode == 'TEST':
        starts = list(range(TRAIN_NUM - P.TIMESTEP_IN,  data.shape[0] - P.TIMESTEP_OUT - P.TIMESTEP_IN + 1))
        for i in starts:
            x = data[i:i+P.TIMESTEP_IN, :]
            y = data[i:i+P.TIMESTEP_IN, :]
    ...
    # ---- diagnostic prints (no logic change) ----------------------------
    if starts:
        covered = set()
        for i in starts:
            covered.update(range(int(i), int(i) + int(P.TIMESTEP_IN)))
        cov_lo, cov_hi = min(covered), max(covered) + 1
        # Stash covered set on a function attribute so we can compare across calls.
        prev = getattr(getXSYS, "_covered_by_mode", {})
        prev[mode] = covered
        getXSYS._covered_by_mode = prev  # type: ignore[attr-defined]
        print(
            f"[getXSYS:{mode}] T_total={data.shape[0]} TRAIN_NUM={TRAIN_NUM} "
            f"TIMESTEP_IN={P.TIMESTEP_IN} TIMESTEP_OUT={P.TIMESTEP_OUT} "
            f"n_windows={len(starts)} start_min={starts[0]} start_max={starts[-1]} "
            f"covered=[{cov_lo},{cov_hi}) |covered|={len(covered)} "
            f"first8_starts={starts[:8]} last8_starts={starts[-8:]}"
        )
        # Cross-mode overlap check: print once we have both TRAIN and TEST.
        if "TRAIN" in prev and "TEST" in prev:
            inter = prev["TRAIN"] & prev["TEST"]
            print(
                f"[getXSYS:overlap] TRAIN ∩ TEST time_steps={len(inter)} "
                f"(应 = 0; 测试时间步是否在训练里出现)"
            )
    else:
        print(f"[getXSYS:{mode}] EMPTY")
    # ---------------------------------------------------------------------

    XS, YS, MS = np.array(XS), np.array(YS), np.array(MS)
    # X: [B, 1, N, T], Y: [B, T, N, 1], M: [B, T, N, 1]
    XS = XS[:, :, :, np.newaxis]          # [B, T, N, 1]
    YS = YS[:, :, :, np.newaxis]          # [B, T, N, 1]
    MS = MS[:, :, :, np.newaxis]          # [B, T, N, 1]
    XS = XS.transpose(0, 3, 2, 1)         # [B, 1, N, T]
    return XS, YS, MS


class ImputationDataset(Dataset):
    """
    Returns (x_masked, y, loss_mask).
      x_masked: [C, N, T]
      y:        [T, N, 1]
      mask:     [T, N, 1]  (1 = evaluated/imputed positions)
    """
    def __init__(self, X, Y, missing_mask, is_train: bool):
        super().__init__()
        self.X = torch.tensor(X).float()   # [B, C, N, T]
        self.Y = torch.tensor(Y).float()   # [B, T, N, 1]
        self.M = torch.tensor(missing_mask).float()  # [B, T, N, 1]
        ...
        if not is_train:
            ...
            obs = 1.0 - self.M
            self.fixed_extra = (torch.rand(self.M.shape, generator=g) < rate).float() * obs
        else:
            self.fixed_extra = None
    ...
    def __getitem__(self, idx):
        ...
        obs = 1.0 - m
        if self.is_train:
            rate = float(torch.empty(1).uniform_(float(P.MASK_MIN), float(P.MASK_MAX)).item())
            extra = (torch.rand_like(m) < rate).float() * obs
        else:
            extra = self.fixed_extra[idx]
        loss_mask = (m + extra).clamp(0.0, 1.0)  # [T, N, 1]

        x_masked = x.clone()
        # mask main channel only (channel 0): align mask [T,N,1] -> [N,T]
        mt = loss_mask.squeeze(-1).permute(1, 0)  # [N, T]
        x_masked[0:1, :, :] = x_masked[0:1, :, :] * (1.0 - mt.unsqueeze(0))
...
def setups():
    ...
    # test split temporal
    trainXS, trainYS, trainMS = getXSYS(data, 'TRAIN')
    testXS, testYS, testMS = getXSYS(data, 'TEST')
    if P.IS_DESEASONED:
        trainXS_ds, _, _ = getXSYS(data_ds, 'TRAIN')
        testXS_ds, _, _ = getXSYS(data_ds, 'TEST')
    ...
    MS_torch_val = trainMS[P.train_size:P.trainval_size,:,:,:]

    # ---- diagnostic: trn/val/test time-coverage cross-check -------------
    # train and val windows are sliced from the TRAIN portion using the same
    # window-start enumeration in getXSYS. Their start indices are:
    #   trn_starts = [0, 1, ..., P.train_size - 1]
    #   val_starts = [P.train_size, ..., P.trainval_size - 1]
    def _cover(starts):
        s = set()
        for i in starts:
            s.update(range(int(i), int(i) + int(P.TIMESTEP_IN)))
        return s
    trn_starts = list(range(0, P.train_size))
    val_starts = list(range(P.train_size, P.trainval_size))
    trn_cov = _cover(trn_starts)
    val_cov = _cover(val_starts)
    tst_cov = getattr(getXSYS, "_covered_by_mode", {}).get("TEST", set())
    def _summ(name, cov):
        if not cov:
            return f"[setups:{name}] EMPTY"
        return (f"[setups:{name}] |covered|={len(cov)} "
                f"range=[{min(cov)},{max(cov)+1})")
    print(_summ("trn(after val split)", trn_cov))
    print(_summ("val(after val split)", val_cov))
    print(_summ("tst", tst_cov))
    print(f"[setups:overlap] trn ∩ val time_steps={len(trn_cov & val_cov)} "
          f"(理想=0; 当前实现可能有 TIMESTEP_IN-1=11 帧重叠,影响最优 ckpt 选择)")
    print(f"[setups:overlap] trn ∩ tst time_steps={len(trn_cov & tst_cov)} "
          f"(应=0; 测试时间步是否在训练里出现)")
    print(f"[setups:overlap] val ∩ tst time_steps={len(val_cov & tst_cov)} "
          f"(应=0)")
    # ---------------------------------------------------------------------
    # spatial split
    ...
    # ---- diagnostic: spatial-split node overlap (unseen split) ----------
    _u_trn = set(int(x) for x in spatialSplit_unseen.i_trn)
    _u_val = set(int(x) for x in spatialSplit_unseen.i_val)
    _u_tst = set(int(x) for x in spatialSplit_unseen.i_tst)
    _inter_tt = sorted(_u_trn & _u_tst)
    _inter_tv = sorted(_u_trn & _u_val)
    _inter_vt = sorted(_u_val & _u_tst)
    _pct_tst = len(_inter_tt) / max(len(_u_tst), 1) * 100.0
    _pct_val = len(_inter_tv) / max(len(_u_val), 1) * 100.0
    print(f"[spatial:unseen] |trn|={len(_u_trn)} |val|={len(_u_val)} "
          f"|tst|={len(_u_tst)} (sum={len(_u_trn)+len(_u_val)+len(_u_tst)}, N_total={data.shape[1]})")
    print(f"[spatial:unseen] trn ∩ tst : {len(_inter_tt)} 个 "
          f"({_pct_tst:.2f}% of tst; 也就是 {len(_inter_tt)}/{len(_u_tst)} 个 \"未见\"测试节点其实在训练里)")
    print(f"[spatial:unseen] trn ∩ tst IDs: {_inter_tt}")
    print(f"[spatial:unseen] trn ∩ val : {len(_inter_tv)} 个 ({_pct_val:.2f}% of val)")
    print(f"[spatial:unseen] trn ∩ val IDs: {_inter_tv}")
    print(f"[spatial:unseen] val ∩ tst : {len(_inter_vt)} 个")
    if _inter_vt:
        print(f"[spatial:unseen] val ∩ tst IDs: {_inter_vt}")
    # ---------------------------------------------------------------------
    ...
    # torch dataset
    train_data = ImputationDataset(XS_torch_train.numpy(), YS_torch_train.numpy(), MS_torch_train.numpy(), is_train=True)
    val_u_data = ImputationDataset(XS_torch_val_u.numpy(), YS_torch_val_u.numpy(), MS_torch_val_u.numpy(), is_train=False)
    val_a_data = ImputationDataset(XS_torch_val_a.numpy(), YS_torch_val_a.numpy(), MS_torch_val_a.numpy(), is_train=False)
    tst_u_data = ImputationDataset(XS_torch_tst_u.numpy(), YS_torch_tst_u.numpy(), MS_torch_tst_u.numpy(), is_train=False)
    tst_a_data = ImputationDataset(XS_torch_tst_a.numpy(), YS_torch_tst_a.numpy(), MS_torch_tst_a.numpy(), is_train=False)
    # torch dataloader
    ...
    # Geo pretrain node pool: optionally restrict to training nodes.
    if not hasattr(P, "GEO_PRETRAIN_TRAIN_ONLY"):
        P.GEO_PRETRAIN_TRAIN_ONLY = True
    if P.GEO_PRETRAIN_TRAIN_ONLY:
        pool = list(spatialSplit_unseen.i_trn)
    else:
        pool = list(range(data.shape[1]))
    pretrn_iter = random.sample(pool, P.BATCHSIZE)
...
def testModel(name, mode, test_iter, adj_tst, spatialsplit):
    ...
    # Evaluate only on masked positions (missing + fixed extra mask).
    YS = test_iter.dataset.Y.cpu().numpy()          # [B, T, N, 1]
    MS = test_iter.dataset.M.cpu().numpy()          # [B, T, N, 1]
    g = torch.Generator()
    g.manual_seed(int(P.seed))
    rate = float((P.MASK_MIN + P.MASK_MAX) / 2.0) if hasattr(P, "MASK_MIN") else 0.3
    ...
    # inverse_transform expects last dim == N
    YS_btn = np.squeeze(YS).transpose(0, 1, 2)  # [B, T, N]
    YP_btn = np.squeeze(YS_pred).transpose(0, 1, 2)
    shp = YS_btn.shape
    YS_inv = scaler.inverse_transform(YS_btn.reshape(-1, shp[2])).reshape(shp)
    YP_inv = scaler.inverse_transform(YP_btn.reshape(-1, shp[2])).reshape(shp)
    np.save(P.PATH + '/' + P.MODELNAME + '_' + mode + '_' + name +'_prediction.npy', YP_inv)
    ...
    mask2 = np.squeeze(LOSS_MASK).astype(bool)  # [B, T, N]
    gt = YS_inv[mask2]
    pr = YP_inv[mask2]
    if gt.size == 0:
        ...
        denom = np.clip(np.abs(gt), 1e-6, None)
...
def get_argv():
    ...
    P.PRETRN_MODE = sys.argv[27] if len(sys.argv) >= 28 else 'dual'#'dual'#'temporal'#('dual' if P.IS_DUAL_PRETRN else 'geo')
    ...
    # If true, geo pretrain samples only training nodes; else all nodes.
    P.GEO_PRETRAIN_TRAIN_ONLY = bool(int(sys.argv[35])) if len(sys.argv) >= 36 else True
...
def main():
    ...
    # Keep time/spatial splits unchanged, but fit scaler ONLY on training time range
    # and training nodes (unseen split) to avoid test-time statistic leakage.
    if P.seed_SS == -1:
        P.seed_SS = P.seed
    TRAIN_NUM = int(data.shape[0] * P.TRAINRATIO)
    spatialSplit_unseen = unseen_nodes.SpatialSplit(
        data.shape[1], r_trn=P.R_TRN, r_val=.1, r_tst=.2, seed=P.seed_SS
    )
    fit_block = data[:TRAIN_NUM, :][:, spatialSplit_unseen.i_trn]
    scaler = StandardScaler()
    scaler.fit_transform(fit_block)  # fit stats
    # transform all data using fitted stats
    data = (data - scaler.u) / scaler.z
    print('raw data stats(min/max/mean/std)',
          float(np.nanmin(data * scaler.z + scaler.u)),
          float(np.nanmax(data * scaler.z + scaler.u)),
          float(np.nanmean(data * scaler.z + scaler.u)),
          float(np.nanstd(data * scaler.z + scaler.u)))
    print('scaler(u,z)', float(scaler.u), float(scaler.z))

    # de-season scaler
    if P.IS_DESEASONED:
        fit_block_ds = data_ds[:TRAIN_NUM, :][:, spatialSplit_unseen.i_trn]
        scaler_ds = StandardScaler()
        scaler_ds.fit_transform(fit_block_ds)
        data_ds = (data_ds - scaler_ds.u) / scaler_ds.z
