...
SEEDS_GLOBAL=(42 88 250 555 999)
...
SCHED_LOG="${LOGROOT}/scheduler_geotopo_global5_geoF2to5_gpu0235.log"
mkdir -p "$LOGROOT"

log() { printf '%s\n' "$*" | tee -a "$SCHED_LOG"; }

LOCK_FILE="${LOGROOT}/.runner.lock"
exec 9>"$LOCK_FILE"
if ! flock -n 9; then
  log "[exit] another runner is active, skip this launch."
  exit 0
fi
...
run_one() {
  ...
  local statusf="${LOGROOT}/${ds}/${seed}/est/A_geo_topo_f${feat}_s${seed}.status"
  ...
  printf '%s\n' "launched_at=$(date '+%F %T')" >"$statusf"
  if CUDA_VISIBLE_DEVICES="${gpu}" \
      MOE_EXPERTS="geo,topo" \
      MOE_TOP_K="2" \
      MOE_RUN_TAG="est_vnode_split_${ds,,}_geo_topo_f${feat}_s${seed}" \
      "$PYBIN" "$SCRIPT" "${BASE[@]}" "${EXTRA_TOPO[@]}" >"$logf" 2>&1; then
    :
  else
    printf '%s\n' "last_exit=nonzero" >>"$statusf"
  fi

  if is_done "$logf"; then
    printf '%s\n' "state=done" >>"$statusf"
    log "[done] ${ds} seed=${seed} feat=${feat}"
  else
    printf '%s\n' "state=incomplete" >>"$statusf"
    log "[incomplete] ${ds} seed=${seed} feat=${feat} (will retry on next pass)"
  fi
}

tasks=()
for ds in "${DATASETS[@]}"; do
  for seed in "${SEEDS_GLOBAL[@]}"; do
