#!/usr/bin/env python3
"""Aggregate virtual-node splitmask estimation logs for one dataset."""
...
def resolve(logroot: Path, dataset: str, seed: int, key: str) -> tuple[str, dict[str, Metrics]]:
...
def metric_table(logroot: Path, dataset: str, split: str) -> list[str]:
    rows = [
        f"## `{split}` 汇总（均值 ± 样本标准差）",
        "",
        "| 配置 | n | MAE | RMSE | MAPE |",
    ...
    for key, label in CONFIGS:
        maes, rmses, mapes = [], [], []
        for seed in SEEDS:
            st, ms = resolve(logroot, dataset, seed, key)
            if st != "完成":
                continue
...
def default_logroot(dataset: str) -> str:
    return f"logs_topomoe/est_{dataset.lower()}_virtualnode_splitmask_seed10"


def default_out(dataset: str) -> str:
    return f"{dataset}_virtualnode_splitmask_Estimation.md"


def main() -> None:
    ap = argparse.ArgumentParser()
    ap.add_argument("--dataset", required=True, choices=("PEMSBAY", "PEMSD7M", "METRLA"))
    ...
    logroot = (
        ...
        else (args.logroot.resolve() if args.logroot is not None else (root / default_logroot(dataset)).resolve())
    )
    out = (
        (root / args.out).resolve()
        if args.out is not None and not args.out.is_absolute()
        else (args.out.resolve() if args.out is not None else (root / default_out(dataset)).resolve())
    )

    md: list[str] = []
    md.append(f"# {dataset} Virtual-Node SplitMask Estimation（10 seeds，无 SCPT）")
    md.append("")
    ...
    md.append("- **运行脚本**：`./run_pems2ds_virtualnode_splitmask_7gpu.sh`")
    ...
    for split in ("tst_u", "tst_v", "tst_a"):
        md.extend(metric_table(logroot, dataset, split))
    ...
    for seed in SEEDS:
        ...
        for key, label in CONFIGS:
            p = logroot / dataset / str(seed) / "est" / f"{key}_s{seed}.log"
            _, ms = resolve(logroot, dataset, seed, key)
